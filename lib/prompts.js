...
/**
 * Rewrite an image prompt (possibly comma-separated SD tags) into one vivid
 * natural-language paragraph, preserving every concrete detail. Used by
 * chart-art --rewrite; shares the Venice chat plumbing above.
 */
export const rewriteAsNaturalLanguage = async ({ prompt, rating = "R", model = "zai-org-glm-4.6", debug = false }) => {
  if (!prompt || !prompt.trim()) {
    throw new Error("rewriteAsNaturalLanguage: prompt is required");
  }
  if (SMOKE_MODE) return `[mock ${rating} natural] ${prompt.trim()}`;
  if (!process.env.VENICE_API_TOKEN) {
    throw new Error("VENICE_API_TOKEN is not set; required for prompt rewriting (uses Venice text models).");
  }
    ...
    messages: [
      {
        ...
        content: [
          "You are a prompt engineer for AI image generation models.",
          "Rewrite the user's image prompt — which may be a comma-separated list of Stable Diffusion tags — into ONE vivid English prompt in flowing natural language. Preserve every concrete detail (subject, clothing, setting, style, lighting, mood); do not invent major new elements.",
          "Output a single paragraph of plain text. No markdown, no quotation marks, no preamble, no trailing notes.",
          `Content rating: ${rating}. ${guidance}`,
          "Regardless of rating, avoid directly sexualized or explicit language: when the source prompt leans that way, translate it into artistic description — form, light, composition, mood — rather than explicit acts or anatomy.",
        ].join(" "),
      },
  ...
  if (!response.ok) {
    throw new Error(`Venice chat API ${response.status}: ${await response.text()}`);
  }
  const data = await response.json();
