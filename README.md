...
- **Prompt pool comes from aiwdm itself** (since 2026-08-05): a wrangler `d1 execute` against the aiwdm D1 selects every image/video with **score ≥ 7** whose prompt text is **longer than 40 chars** — `metadata.prompt` when present, else `description` (for the Grok-export rows the description *is* the original prompt; for wave-cli uploads it's the Venice AI description, which reads as a prompt). Exact-duplicate texts are deduped, the pool is shuffled with no repeats within a run, and thresholds are overridable via `AIWDM_MIN_SCORE` / `AIWDM_MIN_PROMPT_CHARS` in the env file. The query needs `CLOUDFLARE_API_TOKEN` in `~/.config/diem-burner/env` (the systemd unit doesn't read `~/.zshrc`) and is killed after 60 s so a hung wrangler can't eat the burn window. **Fallback:** if the query fails or matches nothing, the old pool — random `~/prompts/**/*.txt` files (all subfolders except `old/`, `short/`, `images/`, dotdirs, `node_modules`) — takes over, so a Cloudflare hiccup never kills an unattended night. Either way it shells out to `venice --prompt <text-or-file> --model <id> --format <ratio> --resolution 1K --aiwdm-tags diem-burner`, so every image auto-uploads to aiwdm tagged `venice, <model>, diem-burner`. The format is drawn at random per image from `DIEM_BURNER_FORMATS` in the env file (comma-separated aspect ratios; currently `9:16`); `--resolution 1K` is pinned because tier-priced models otherwise bill their *default* tier (seedream-v5-pro defaults to 2K: 0.11 instead of 0.06 — verified empirically).
